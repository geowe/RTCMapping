...
RealTimeEngine.prototype.emitAttributeNewValue = function(featureId, attributeName, attributeValue){    
  this.socket.emit(RteOperation.INFO_TOOL,
    {featureId: featureId, column: attributeName, value: attributeValue, room: this.appContext.getRoom()});
}
...
RealTimeEngine.prototype.emitNewAttribute = function(attributeName){    
  this.socket.emit(RteOperation.NEW_ATTRIBUTE,
    {newColumn: attributeName,  room: this.appContext.getRoom()});
}
...
RealTimeEngine.prototype.emitTitle = function(title){    
  this.socket.emit(RteOperation.DOC_TITLE,
    {title: title,  room: this.appContext.getRoom()});
}
...
RealTimeEngine.prototype.listen = function() {      
    ...
    function showUrlToShare(nick, room){
      if(isOwner){          
        var urlGenerator = new SharedUrlGenerator({nick: nick, room: room,
          center: map.getView().getCenter(), zoom: map.getView().getZoom()});
    ...
    this.socket.on(RteOperation.MODIFY, function (geojson) {           
      var feature = buildFeature(geojson);
      source.removeFeature(source.getFeatureById(feature.getId()));
      source.addFeature(feature);
      new Notificator().notify('Se ha Modificado el elemento: '+ feature.getId()); 
    });
